...
func init() {
	...
	*_logFileName = func() string {
		if *_logFileName == "" {
			return fmt.Sprintf("report-product-%v", *_id)
		}
		return *_logFilePath
	}()
}
...
func main() {
	service, err := product.NewService(
		product.WithId(*_id),
		product.WithHost(*_host),
		product.WithHTTPPort(*_httpPort),
		product.WithRPCPort(*_rpcPort),
		product.WithVersion(*_version),
		product.WithNacosScheme(_scheme),
		product.WithNacosHost(*_nacosHost),
		product.WithNacosGroup(*_nacosGroup),
		product.WithNacosLogLevel(*_nacosLogLevel),
		product.WithLogFilePath(*_logFilePath),
		product.WithLogFileName(*_logFileName),
		product.WithNacosLogDir(*_nacosLogPath),
		product.WithNacosHttpPort(*_nacosHttpPort),
		product.WithNacosGrpcPort(*_nacosGrpcPort),
		product.WithNacosCacheDir(*_nacosCachePath),
		product.WithRegisterTTL(registry.DefaultTTL),
		product.WithNacosNamespaceId(*_nacosNamespaceId),
		product.WithConfig(config.RPCGroup, config.ServerDataId),
		product.WithConfig(config.RouteGroup, config.ProtocolDataId),
		product.WithConfig(config.ServiceGroup, service2.Product),
		product.WithRegisterInterval(registry.DefaultInterval),
		product.WithConfig(config.ComponentGroup, config.BrokerDataId),
		product.WithConfig(config.ComponentGroup, config.MySQLDataId),
		product.WithConfig(config.ComponentGroup, config.RedisDataId),
		product.WithConfig(config.ComponentGroup, config.MongoDBDataId),
		product.WithConfig(config.NotifierGroup, service2.Product),
		product.WithConfig(config.SecurityGroup, config.GracefulShutdownDataId),
		product.WithNotifyBufferSize(service2.DefaultNotifyBufferSize),
		product.WithForwardChannelBufferSize(service2.DefaultForwardChannelBufferSize),
	)
	if err != nil {
		panic(err)
	}
