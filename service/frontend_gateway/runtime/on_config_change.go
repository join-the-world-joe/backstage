...
func listen(cf config.Config) {
	cf.Subscribe(config.Parameter{
		...
		DataId:   service.FrontendGateway,
		OnChange: servant,
	})
	cf.Subscribe(config.Parameter{
	...
	cf.Subscribe(config.Parameter{
		Group:    config3.SecurityGroup,
		DataId:   config3.RateLimitingDataId,
		OnChange: config2.RateLimiting,
	})
}
...
func servant(namespace, group, dataId, data string) {
	...
	serviceConf.Servant.WaitForCloseInterval = temp.Servant.WaitForCloseInterval
