...
var googleSecretfool = string("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567")

//秘钥生成
func CreateGoogleSecret(key string) string {
	...
	secretfool := md5key[0:16]
	if len(secretfool) != 16 {
		return secret
	}
	tmpsecret := string("")
	for _, v := range secretfool {
		num := strToInt(fmt.Sprintf("%x", v)) % 32
		tmpsecret += string([]byte(googleSecretfool)[num])
	}
	secret = tmpsecret
	return secret
}
...
//谷歌验证
func GetGoogleCode(secret string) (int32, int64) {
	...
	return int32(code), secondsRemaining
}
...
func strToInt(src string) int {
	_val, _ := strconv.ParseInt(src, 10, 64)
	return int(_val)
}