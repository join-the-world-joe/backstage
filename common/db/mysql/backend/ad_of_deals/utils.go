...
func InsertModel(model *Model) (*Model, error) {
	...
	model, ok := temp.(*Model)
	if !ok {
		return nil, fmt.Errorf("ad_of_deals.insert failure, convert to Model fail")
	}
	if model.Id == 0 {
		return nil, fmt.Errorf("ad_of_deals.insert failure, model.Id == 0")
	}
	return model, nil
}
...
func RemoveOutdatedRecordsOfADOfDeals(version int64) error {
