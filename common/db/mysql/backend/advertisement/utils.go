...
func InsertModel(model *Model) (*Model, error) {
	...
	model, ok := temp.(*Model)
	if !ok {
		return nil, fmt.Errorf("product_advertisement.insert failure, convert to Model fail")
	}
	if model.Id == 0 {
		return nil, fmt.Errorf("product_advertisement.insert failure, model.Id == 0")
	}
	return model, nil
}
...
func GetModelListByTitle(title string) ([]*Model, error) {
	modelList := []*Model{}
	db, err := mysql.GetDB(GetWhich(), GetDbName())
	if err != nil {
		return nil, err
	}
	err = db.Table(GetTableName()).Where("title = ? AND visible = 1", title).Find(&modelList).Error
	if err != nil {
		return nil, err
	}
