...
func GetModelListByProductId(productId int64) ([]*Model, error) {
	modelList := []*Model{}
	db, err := mysql.GetDB(GetWhich(), GetDbName())
	if err != nil {
		return nil, err
	}
	err = db.Table(GetTableName()).Where("product_id = ? AND visible = 1", productId).Find(&modelList).Error
	if err != nil {
		return nil, err
	}
...
func UpdateVisibleByProductIdAndSellingPoint(productId int64, sellingPoint string, visible int) error {
	db, err := mysql.GetDB(GetWhich(), GetDbName())
	if err != nil {
		return err
	}
	return db.Table(GetTableName()).Where(fmt.Sprintf("id = %v AND selling_point = '%v' AND visible = 1", productId, sellingPoint)).Updates(map[string]interface{}{"visible": visible}).Error
}
