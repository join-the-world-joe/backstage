...
func Insert(model *Model) (*Model, error) {
	...
	model, ok := temp.(*Model)
	if !ok {
		return nil, fmt.Errorf("ad_of_deals_of_today.insert failure, convert to Model fail")
	}
	if model.Id == 0 {
		return nil, fmt.Errorf("ad_of_deals_of_today.insert failure, model.Id == 0")
	}
	return model, nil
}
