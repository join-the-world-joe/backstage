...
func InsertModel(model *Model) (*Model, error) {
	...
	model, ok := temp.(*Model)
	if !ok {
		return nil, fmt.Errorf("version_of_ad_of_bbq_products.insert failure, convert to Model fail")
	}
	if model.Id == 0 {
		return nil, fmt.Errorf("version_of_ad_of_bbq_products.insert failure, model.Id == 0")
	}
	return model, nil
}
